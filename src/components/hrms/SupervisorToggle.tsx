...
export default function SupervisorToggle() {
    ...
    return (
        <div className="flex items-center gap-3 bg-blue-50 px-4 py-2 rounded-lg border border-blue-100">
            <span className={`text-sm font-medium ${isSupervisorView ? 'text-blue-700' : 'text-gray-500'}`}>
                Supervisor View
            </span>
            <button
                onClick={toggleView}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${isSupervisorView ? 'bg-blue-600' : 'bg-gray-200'
                    }`}
                role="switch"
